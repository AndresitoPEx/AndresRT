...
- **Theming** ([assets/js/theme-toggle.js](assets/js/theme-toggle.js)) — `ThemeManager` sets/removes `data-theme="dark"` on `<html>` and persists to localStorage. **Light is the default and the flagship design** (SaaS-consultant look: white surfaces, slate ink, `#226DB4` accent); dark mode keeps the older navy design as a variant. System preference is intentionally not followed.
- **Projects** ([assets/js/github-projects.js](assets/js/github-projects.js)) — `GitHubProjects` fetches repos from the GitHub API for user `AndresitoPEx` at runtime, filters/orders them by the hardcoded `featuredProjects` whitelist (by repo name, forks excluded), and renders minimal "pinned repo" cards into the static `.repo-grid`. The repos have no GitHub descriptions, so card copy comes from the bilingual `this.descriptions` map in the same file (re-rendered on `languageChanged`). To feature/unfeature a project, edit the whitelist.
...
- Font Awesome is vendored locally as `all.min.*` — never edit those files. (Swiper was removed from the page when the projects carousel became a static grid; `swiper-bundle.min.*` and `slider.js` remain on disk but are no longer referenced.)

CSS is one file per section (`header.css`, `titular.css`, `services.css`, `exp.css`, `summary.css`, `contact-modern.css`, …), each linked from `<head>`. Base variables and resets live in `base.css` / `reset.css`. Two overlay stylesheets load last, in order: `zoho-accent.css` (open, line-based layouts over the per-section files) and `theme-saas.css` (the light-first SaaS theme: redefines the `--senior-*`/`--color-*` variables for light mode under `html:not([data-theme="dark"])`, plus the terminal-style details — JetBrains Mono labels, `$` prefixes and window dots on case cards — that apply in both modes). Fonts are Inter (body/headings) and JetBrains Mono (labels/metrics/tags), imported in `base.css`. When changing colors, prefer redefining variables in `theme-saas.css` over editing the per-section files.

Page sections in order: hero (`titular` + summary modal + metrics strip), `#sobre-mi`, `#servicios`, `#skills` (two tiers: Zoho specialty + complementary web dev), `#formacion`, `#exp-prof` (work experience), `#casos` (hand-written Zoho case studies — problem/solution/result, styled by `case-studies.css`; the nav "Proyectos" item points here), `#exp` (GitHub projects grid, demoted to secondary), `#contacto`, footer.
